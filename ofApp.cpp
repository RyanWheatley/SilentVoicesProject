...
void ofApp::audioOut(float *output, int bufferSize, int nChannels)
{
	...
	if (playingEnabled) {
		for (int i = 0; i < bufferSize; i++) {
			...
			switch (playCount) {
			case '0':
				output[2 * i] = buffer[playPos] * (1.0 - playCount / 10.0); // playing sound to first speaker
				output[2 * i + 1] = 0;        //playing no sound from rest of speakers
				break;

			case '1':
				output[2 * i] = 0;
				output[2 * i + 1] = buffer[playPos] * (1.0 - playCount / 10.0); //  playing speach sample to second speaker at -10% amplitude   
				break;



		   //case '2' :  //when we add the third, fourth,..  speakers
		   //   output[ 2*i ]  =  output[ 2*i + 1 ] = 0;     //playing no sound from rest of speakers
	       //   output[ 2*i+2 ] = buffer[ playPos ] * (1.0 - playCount/10.0); // playing speach sample to thrird speaker at -20% amplitude  
		   //   break;
			}
